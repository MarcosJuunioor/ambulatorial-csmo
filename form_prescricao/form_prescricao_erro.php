...
$old_error_handler = set_error_handler("form_prescricao_scriptcase_error_handler");

function form_prescricao_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_prescricao']['contr_erro']) && $_SESSION['scriptcase']['form_prescricao']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_prescricao->contr_form_prescricao->Apl_com_erro = true;
            if ($inicial_form_prescricao->contr_form_prescricao->NM_ajax_flag)
            {
                $inicial_form_prescricao->contr_form_prescricao->NM_ajax_info['errList']['geral_form_prescricao'][] = $msg;
                return;
            }
            form_prescricao_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_prescricao_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_prescricao'])  && '' != $_SESSION['scriptcase']['error_icon']['form_prescricao'])  ? $_SESSION['scriptcase']['error_icon']['form_prescricao']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_prescricao']) && '' != $_SESSION['scriptcase']['error_close']['form_prescricao']) ? $_SESSION['scriptcase']['error_close']['form_prescricao'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_prescricao']) && '' != $_SESSION['scriptcase']['error_span_title']['form_prescricao']) ? $_SESSION['scriptcase']['error_span_title']['form_prescricao'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_prescricao']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_prescricao']) ? $_SESSION['scriptcase']['error_icon_title']['form_prescricao'] : '';
?>
