...
<HEAD>
 <TITLE><?php if ('novo' == $this->nmgp_opcao) { echo strip_tags("" . $this->Ini->Nm_lang['lang_othr_frmi_titl'] . ""); } else { echo strip_tags("Ficha de acompanhamento"); } ?></TITLE>
...
?>
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>control_ficha_de_acompanhamento/control_ficha_de_acompanhamento_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
<?php
include_once("control_ficha_de_acompanhamento_mob_sajax_js.php");
?>
<script type="text/javascript">
...
include_once('control_ficha_de_acompanhamento_mob_jquery.php');
 ...
 function addAutocomplete(elem) {
  ...
  $(".sc-ui-autocomp-nome", elem).focus(function() {
   ...
   source: function (request, response) {
    $.ajax({
     url: "control_ficha_de_acompanhamento_mob.php",
     dataType: "json",
     data: {
      term: request.term,
   ...
   change: function (event, ui) {
    var sId = $(this).attr("id").substr(6), sRow = 'nome' != sId ? sId.substr(4) : '';
    if ("" == $(this).val()) {
     do_ajax_control_ficha_de_acompanhamento_mob_event_nome_onchange(sRow);
    }
   },
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['recarga'];
}
?>
...
<?php
 include_once("control_ficha_de_acompanhamento_mob_js0.php");
?>
<script type="text/javascript"> 
...
<?php
$_SESSION['scriptcase']['error_span_title']['control_ficha_de_acompanhamento_mob'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['control_ficha_de_acompanhamento_mob'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<?php
  if (!$this->Embutida_call && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['mostra_cab']) || $_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['mostra_cab'] != "N"))
  {
?>
<tr><td>
<style>
...
<div style="width: 100%">
 ...
 <div style="height:37px; border-width:0px 0px 1px 0px;  border-style: dashed; border-color:#ddd; display: block">
 	<table style="width:100%; border-collapse:collapse; padding:0;">
    	<tr>
        	<td id="lin1_col1" class="scFormHeaderFont"><span><?php if ($this->nmgp_opcao == "novo") { echo "" . $this->Ini->Nm_lang['lang_othr_frmi_titl'] . ""; } else { echo "Ficha de acompanhamento"; } ?></span></td>
...
<?php
  ...
  {
       if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento']['where_filter']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['empty_filter'] = true;
       }
  }
  else
  {
...
<TABLE align="center" id="hidden_bloco_0" class="scFormTable" width="100%" style="height: 100%;"><?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    ...
    {
        $this->nm_new_label['nome'] = "Nome do aluno";
    }
?>
<?php
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOdd css_curso_line" id="hidden_field_data_curso" style="<?php echo $sStyleHidden_curso; ?>"> <span class="scFormLabelOddFormat css_curso_label"><span id="id_label_curso"><?php echo $this->nm_new_label['curso']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["curso"]) &&  $this->nmgp_cmp_readonly["curso"] == "on") { 

 ?>
<input type="hidden" name="curso" value="<?php echo $this->form_encode_input($curso) . "\">" . $curso . ""; ?>
<?php } else { ?>
<span id="id_read_on_curso" class="sc-ui-readonly-curso css_curso_line" style="<?php echo $sStyleReadLab_curso; ?>"><?php echo $this->form_encode_input($this->curso); ?></span><span id="id_read_off_curso" style="white-space: nowrap;<?php echo $sStyleReadInp_curso; ?>">
 <input class="sc-js-input scFormObjectOdd css_curso_obj" style="" id="id_sc_field_curso" type=text name="curso" value="<?php echo $this->form_encode_input($curso) ?>"
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOdd css_modalidade_line" id="hidden_field_data_modalidade" style="<?php echo $sStyleHidden_modalidade; ?>"> <span class="scFormLabelOddFormat css_modalidade_label"><span id="id_label_modalidade"><?php echo $this->nm_new_label['modalidade']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["modalidade"]) &&  $this->nmgp_cmp_readonly["modalidade"] == "on") { 

 ?>
<input type="hidden" name="modalidade" value="<?php echo $this->form_encode_input($modalidade) . "\">" . $modalidade . ""; ?>
<?php } else { ?>
<span id="id_read_on_modalidade" class="sc-ui-readonly-modalidade css_modalidade_line" style="<?php echo $sStyleReadLab_modalidade; ?>"><?php echo $this->form_encode_input($this->modalidade); ?></span><span id="id_read_off_modalidade" style="white-space: nowrap;<?php echo $sStyleReadInp_modalidade; ?>">
 <input class="sc-js-input scFormObjectOdd css_modalidade_obj" style="" id="id_sc_field_modalidade" type=text name="modalidade" value="<?php echo $this->form_encode_input($modalidade) ?>"
...
<?php } else { ?>
<span id="id_read_on_data_nascimento" class="sc-ui-readonly-data_nascimento css_data_nascimento_line" style="<?php echo $sStyleReadLab_data_nascimento; ?>"><?php echo $this->form_encode_input($this->data_nascimento); ?></span><span id="id_read_off_data_nascimento" style="white-space: nowrap;<?php echo $sStyleReadInp_data_nascimento; ?>">
...
<input type=hidden name="turno" value="<?php echo $this->form_encode_input($this->turno) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>
...
$turno_look = "";
 if ($this->turno == "manha") { $turno_look .= "Manhã" ;} 
 if ($this->turno == "tarde") { $turno_look .= "Tarde" ;} 
 if ($this->turno == "noite") { $turno_look .= "Noite" ;} 
 if (empty($turno_look)) { $turno_look = $this->turno; }
?>
<input type="hidden" name="turno" value="<?php echo $this->form_encode_input($turno) . "\">" . $turno_look . ""; ?>
<?php } else { ?>
<?php
...
$turno_look = "";
 ...
 if (empty($turno_look)) { $turno_look = $this->turno; }
?>
<span id="id_read_on_turno" class="css_turno_line"  style="<?php echo $sStyleReadLab_turno; ?>"><?php echo $this->form_encode_input($turno_look); ?></span><span id="id_read_off_turno" style="<?php echo $sStyleReadInp_turno; ?>">
 <span id="idAjaxSelect_turno"><select class="sc-js-input scFormObjectOdd css_turno_obj" style="" id="id_sc_field_turno" name="turno" size="1" alt="{type: 'select', enterTab: false}">
 <option value="manha" <?php  if ($this->turno == "manha") { echo " selected" ;} ?><?php  if (empty($this->turno)) { echo " selected" ;} ?>>Manhã</option>
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['Lookup_turno'][] = 'manha'; ?>
 <option value="tarde" <?php  if ($this->turno == "tarde") { echo " selected" ;} ?>>Tarde</option>
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['Lookup_turno'][] = 'tarde'; ?>
 <option value="noite" <?php  if ($this->turno == "noite") { echo " selected" ;} ?>>Noite</option>
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['Lookup_turno'][] = 'noite'; ?>
 </select></span>
</span><?php  }?>
 </TD>
   <?php }?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOdd css_texto_line" id="hidden_field_data_texto" style="<?php echo $sStyleHidden_texto; ?>"> <span class="scFormLabelOddFormat css_texto_label"><span id="id_label_texto"><?php echo $this->nm_new_label['texto']; ?></span></span><br>
<?php
...
<span id="id_read_on_texto" class="sc-ui-readonly-texto css_texto_line" style="<?php echo $sStyleReadLab_texto; ?>"><?php echo $this->form_encode_input($texto_val); ?></span><span id="id_read_off_texto" style="white-space: nowrap;<?php echo $sStyleReadInp_texto; ?>">
 <textarea  class="sc-js-input scFormObjectOdd css_texto_obj" style="white-space: pre-wrap;" name="texto" id="id_sc_field_texto" rows="34" cols="75"
 alt="{datatype: 'text', maxLength: 2550, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}">
<?php echo $texto; ?>
</textarea>
</span><?php } ?>
 </TD>
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_ficha_de_acompanhamento_mob']['masterValue']))
{
?>
...
?>
<script>
 parent.scAjaxDetailStatus("control_ficha_de_acompanhamento_mob");
 parent.scAjaxDetailHeight("control_ficha_de_acompanhamento_mob", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
}
